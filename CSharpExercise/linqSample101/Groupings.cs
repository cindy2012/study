...
namespace Try101LinqSamples
{
    public class Groupings
    {
        ...
        public int GroupingSyntax()
        {
            ...
            var numberGroups2 = numbers.GroupBy(x => x % 5).Select(x => new { Remainder = x.Key, Numbers = x });
        ...
        public int GroupByProperty()
        {
            ...
            var wordGroups2 = words.GroupBy(x => x.First()).Select(x => new { FirstLetter = x.Key, Words = x });
        ...
        public int GroupByCategory()
        {
            ...
            var orderGroups2 = products.GroupBy(x => x.Category).Select(m => new { Category = m.Key, Products = m });
            ...
            Console.WriteLine("+++++++++++++++++");
            foreach (var orderGroup in orderGroups2)
            {
                Console.WriteLine($"Products in {orderGroup.Category} category:");
                foreach (var product in orderGroup.Products)
                {
        ...
        public int NestedGrouBy()
        {
            ...
            var customerOrderGroups2 = customers.Select(x => new
            {
                CompanyName = x.CompanyName
              ,
                YearGroups = x.Orders.Where(x=>x.OrderDate.Year==1998).GroupBy(m => m.OrderDate)
              .Select(x => new
              {
                  Year = x.Key,
                  MonthGroups =
              x.GroupBy(c => c.OrderDate.Month).Select(dd => new { Month = dd.Key, Orders = dd })
              })
            });
            foreach (var ordersByCustomer in customerOrderGroups2)
            {
